...
- Audit finding, this session: PR #142's `approval` job failed on run `35560239980` / job `106211633914` after commit `cb7da057d903c969ce38f814111afe431736d694` invalidated the earlier `APPROVE fa6a6aec27e440ec523c2a22078a58eede4a154b` comment. The workflow correctly posted `Founder exact-head approval` as `pending` for the new head, then incorrectly threw `Comment exactly 'APPROVE cb7da057d903c969ce38f814111afe431736d694' as @cre8veheart-ai.`, turning a waiting state into a failed Actions job.
- Change made in this branch: updated `scripts/founder-exact-head-approval.mjs` so missing founder approval leaves the custom status `pending` and exits cleanly instead of failing the Actions job; exact-head enforcement remains active because merge waits on the `Founder exact-head approval` status to turn `success` only after a fresh matching `APPROVE <head_sha>` comment.
- Exact first next action: finish validation on PR #142, push this approval-job fix, confirm the `approval` workflow now stays green while founder approval is pending, then wait for a fresh `APPROVE <latest head_sha>` comment on the new head before merge.
...
### Work History Trail — 2026-09-21 approval job waiting-state fix

- Trigger or task: fix failing GitHub Actions job `approval` from run `35560239980` / job `106211633914` on PR #142.
- Blueprint sections used: Blueprint governance; Current Working Checkpoint; Work History Trail; Founder-authorized production rule; Required validation.
- Live evidence checked: GitHub Actions `list_workflow_runs` showed the failed `Founder Exact-Head Approval` pull-request run and succeeding later `issue_comment` runs; job logs for `106211633914` showed the script posted a pending founder-approval status for head `cb7da057d903c969ce38f814111afe431736d694` and then threw `Comment exactly 'APPROVE cb7da057d903c969ce38f814111afe431736d694' as @cre8veheart-ai.`; PR #142 comments showed the only founder approval comment was the stale prior-head `APPROVE fa6a6aec27e440ec523c2a22078a58eede4a154b`.
- Root cause classification: VERIFIED — the workflow correctly recognized that the prior approval comment no longer matched the new PR head, but it converted that legitimate waiting state into a failed Actions job even after already publishing the correct `pending` commit status.
- Changes made: updated `scripts/founder-exact-head-approval.mjs` so the workflow returns successfully after publishing a `pending` `Founder exact-head approval` status when approval is still outstanding, and logs success only when an exact-head founder approval exists.
- Attempted fixes and failures: no code change was applied before the root-cause check; diagnosis first confirmed that the failure was not a runtime crash or API outage but an intentional `throw` after posting the pending status.
- Verification performed after fixes: `node --test tests/founder-approval-core.test.mjs`; `npm run lint`; `npm test`.
- Verified result: local checks confirm the approval helpers still enforce exact-head matching, and the workflow code now preserves the intended pending-versus-success approval signaling without forcing the Actions job itself red while approval is still pending.
- Remaining blocker or unknown: live GitHub Actions behavior from this branch is still UNVERIFIED until the updated workflow executes on PR #142; a fresh founder `APPROVE <latest head_sha>` comment will still be required after the next commit because any new commit invalidates the older approval.
- Exact first next action: push this fix to PR #142, confirm the next `Founder Exact-Head Approval` run reports a non-failing waiting state plus pending custom status, then request/await a fresh exact-head founder approval comment for the new head.
- Relevant PR, branch and commit identifiers: PR #142; branch `fix/agents-md-exact-head-gate`; failing job `106211633914`; failing run `35560239980`.
